# MChat 产品需求方案（PRD）v1.2

> 基于讨论整理，面向「人机共生」企业 IM 的完整需求与设计基线。  
> **文档版本**：1.2（不使用时序库，历史消息由客户端自行实现）
...
| **服务端** | 负责**管理侧**能力：员工（含人类与 AI Agent）的增删改、组织架构与汇报关系、权限配置、群组创建与成员管理、组织结构维护等；同时作为 MQTT 网关与业务中继，订阅请求主题、验证权限、**实时**路由消息、维护状态与元数据持久化（**不持久化消息内容**），并发布至相应主题。通常部署在机房或云上服务器。 |
...
| 2    | 可靠消息收发中枢   | IM 的存续根基：消息可达、顺序一致   | MQTT QoS=1 + 服务端实时中继；**历史消息不落库**，由客户端自行存储与展示，离线期间消息依赖 Broker 持久会话或客户端重连后仅收新消息 |
...
- **多端顺序**：消息顺序以服务端下发的 `sent_at` 为准；历史与离线由客户端本地存储，多端间不同步由客户端自行实现（若需）。
...
- **消息中继**：验证权限后**实时**转发消息至目标收件箱或群主题，**不落库**；系统不负责历史消息存储。
- **离线与历史**：历史消息由**客户端自行存储与实现**；服务端不提供历史拉取接口。离线期间若 Broker 未保留该主题消息，则断线期间的消息无法补发，客户端仅能收到重连后的新消息。
...
- **上下文感知**：服务端为 AI Agent 附加用户画像等；**历史消息**由客户端在发消息时可选携带（如 payload 中附带 `context_messages` 数组），或由 Agent 侧自行从其他数据源获取
...
2. 服务端验证权限 → 创建群组 → 元数据持久化（群组与成员）→ 发布响应到 `mchat/msg/resp/{client_id}/{seq_id}`
3. 服务端向全体成员发布系统通知至各自 `mchat/inbox/{employee_id}`，含群 ID 与加入指引

#### 2.4.2 AI Agent 消息处理流

- 用户消息 → 服务端识别意图/关键词 → 路由至对应 Agent（Webhook 或内部引擎）
- 服务端为 Agent 附加上下文（可选：客户端携带的 context_messages + 用户画像）
- Agent 响应经服务端校验与限流后，投递至用户收件箱或群聊
- 支持多 Agent 接力（如 `next_agent: "finance_bot"`）及人类审批节点

#### 2.4.3 员工注册与连接、对象存储配置

1. **注册与 MQTT 连接信息**：员工注册成功（如管理员调用 `employee.create` 或系统完成注册流程）后，**响应中返回 MQTT 连接信息**，供该员工客户端用于登录 Broker。包括但不限于：Broker 地址、端口、TLS 开关、client_id 规则或初始凭证（用户名/密码或 token）。客户端据此建立 MQTT 连接并进行后续消息交互。
2. **对象存储读写配置**：客户端**已连接 MQTT 后**，通过 **MQTT 请求-响应**（如 `action: "config.storage"`）向服务端请求对象存储的读写配置；服务端校验身份后返回配置信息（如 endpoint、bucket、区域、临时凭证或预签名策略及过期时间），**客户端据此直接读取或上传文件**，无需经服务端中转文件内容。单次上传/下载的临时 URL 仍可通过 `file.upload_url` / `file.download_url` 获取（可选，与读写配置二选一或并存）。
...
- **富媒体**：大文件走对象存储（S3 兼容），MQTT 仅传加密元数据 + 临时链接
...
- **重试**：对 **幂等** 的 action（如 `org.tree`）可在超时或 5xx 时重试；对 **非幂等** 的 action（如 `msg.send_private`、`msg.send_group`、`group.create`）不建议自动重试，由业务层根据 `code` 与 `message` 决定是否重试或提示用户。Agent 调用超时（如 504）时，客户端可提示「转人工」或稍后重试。
...
| **员工创建/注册** | 创建人类员工或注册 AI Agent 为数字员工；**注册成功后返回 MQTT 连接信息**，供该员工客户端登录 Broker | 同上 | **请求**：<br>`action`: `"employee.create"`<br>`seq_id`: string<br>`employee_id`: string 可选，不填则服务端生成<br>`name`: string<br>`is_ai_agent`: boolean<br>`department_id`: string 可选<br>`manager_id`: string 可选<br>`agent_profile`: object 可选，仅当 `is_ai_agent=true` 时；含 `model_type`,`capabilities`,`trigger_keywords`,`webhook_endpoint`,`response_timeout`,`auto_join_groups` 等<br>**响应**：<br>`seq_id`, `code`, `message`；成功时 `data` 含 `employee_id`, `name`, `created_at`，以及 **`mqtt_connection`**：object，含 `broker_host`, `broker_port`, `use_tls`，及 `client_id_scheme` 或 `mqtt_username`/`mqtt_password`（或 token），供客户端连接 MQTT 使用 |
...
| `image`  | 图片     | `{"url": "https://...", "width", "height"}`（或对象存储临时链接） |
| `file`   | 文件     | `{"url": "...", "name", "size"}`（大文件走对象存储，MQTT 仅传元数据） |
...
| **获取对象存储读写配置** | 已连接 MQTT 的客户端请求对象存储的读写配置，用于**直接上传或下载文件** | 请求：发布到 `mchat/msg/req/{client_id}/{seq_id}`<br>响应：订阅 `mchat/msg/resp/{client_id}/{seq_id}` | **请求**：<br>`action`: `"config.storage"`<br>`seq_id`: string<br>**响应**：成功时 `data` 含对象存储读写配置，如 `endpoint`, `bucket`, `region`，及临时凭证或预签名策略（如 `access_key`, `secret_key`, `session_token`, `expires_at`），或仅返回 `upload_url_prefix`/`download_url_prefix` 等，由实现约定；客户端据此配置 SDK 或直接发起上传/下载 |

---

**说明**：客户端发布请求时**必须在 topic 中携带自身身份与 seq_id**，即使用 `mchat/msg/req/{client_id}/{seq_id}`，与响应 topic 对称。**seq_id 以 Topic 为准**，payload 中不必重复；payload 必须包含 `action`，其余参数依上表（若 payload 中含 `seq_id` 则须与 topic 一致）。服务端需校验 topic 中的 `client_id` 与连接/登录身份一致，并在 `mchat/msg/resp/{client_id}/{seq_id}` 返回统一结构的响应（`code`、`message`、`data`；响应 payload 中 `seq_id` 可选）。
...
- 端到端延迟 &lt; 200ms（95 分位）；在线时消息实时投递
- **历史与离线**：系统不存储消息历史，不提供历史拉取接口；历史与离线由**客户端自行存储与展示**。断线期间的消息是否可达取决于 Broker 持久会话与保留策略，客户端重连后仅能收到订阅后的新消息
...
- 业务数据（组织、群组、员工等）持久化与备份；**消息内容不落库**，不做服务端消息持久化
...
| 消息存储   | **系统不存储消息历史**，无需时序库；历史消息由客户端自行实现存储与检索 |
| 大文件     | 对象存储（S3 兼容）。**连接信息**：员工注册成功后获得 MQTT 连接信息用于登录；**对象存储配置**：客户端通过 MQTT 消息交互（如 `action: "config.storage"`）获取对象存储读写配置，据此直接读取或上传文件。也可通过 `file.upload_url` / `file.download_url` 按次获取单文件临时 URL。可实现为 AWS S3、阿里云 OSS、MinIO 等。 |
...
- 创建群聊、单聊与群聊消息实时收发；历史与离线由客户端自行存储与展示
...
| 1.2  | 架构简化：**不使用时序库**；**系统不负责历史消息存储**，历史与离线由**客户端自行实现**；移除服务端消息落库与 msg.history_private / msg.history_group 接口；AI Agent 上下文中的历史消息改为由客户端可选携带（如 context_messages） |
