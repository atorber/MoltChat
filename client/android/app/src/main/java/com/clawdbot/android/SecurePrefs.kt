...
class SecurePrefs(context: Context) {
  ...
  // MChat: 员工 ID，用于 auth.bind 与收件箱订阅
  private val _mchatEmployeeId = MutableStateFlow(prefs.getString("mchat.employeeId", null)?.trim().orEmpty())
  val mchatEmployeeId: StateFlow<String> = _mchatEmployeeId
  ...
  fun setMcHatEmployeeId(value: String) {
    val trimmed = value.trim()
    prefs.edit { putString("mchat.employeeId", trimmed) }
    _mchatEmployeeId.value = trimmed
  }
